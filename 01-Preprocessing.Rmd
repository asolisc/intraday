...
library(scales)      # scaling in ggplot2
...
[see page 7 of Realised Kernels in Practice - Trades and Quotes (2009)]
...
# Compute trades per hour interval
trades_hour_df <- ipc_intraday %>% 
  mutate(tidy_hour = hour(tidy_time)) %>%
  count(tidy_hour, sort = T, name = "trades_per_hour")

# Compute total trades
total_trades <- trades_hour_df %>% pull(trades_per_hour) %>% sum()

# Get first and last hour in the data
first_hour <- trades_hour_df %>% pull(tidy_hour) %>% min()
last_hour <- trades_hour_df %>% pull(tidy_hour) %>% max()

# Make plot
trades_hour_df %>% 
  mutate(perc_trades = trades_per_hour/total_trades) %>% 
  # mutate(tidy_hour = fct_reorder(tidy_hour, .x = trades_per_hour, .desc = T)) %>%
  ggplot(aes(tidy_hour, perc_trades)) +
  geom_col(
    aes(fill = between(x = as.numeric(tidy_hour), 
                       left = 8, 
                       right = 15)),
    show.legend = F) +
  geom_text(aes(label = scales::percent(perc_trades, 
                                       accuracy = .1)),
                                       # suffix = "k",
                                       # scale = 1/1e3)),
            nudge_y = 0.5/100,
  ...
  scale_fill_manual(values = blog_colors) +
  scale_x_continuous(breaks = first_hour:last_hour,
  ...
  labs(
    title = "Prices per Hour in the Sample (%)",
    subtitle = glue::glue("Total observations: {scales::number(total_trades, big.mark = ',')}"),
    x = "Hour",
    y = NULL
  ) +
  theme(
...
ipc_intraday %>% 
  ...
  scale_fill_manual(values = blog_colors) +
  scale_x_continuous(
    breaks = c(seq(0, 55, 5),59),
    expand = expansion(0.009)
    ) +
  scale_y_continuous(
...
Here, the time with most trades is 3:00PM exactly. Following *Liu, Patton, Sheppard (2013)*, we'll discard any prices that were quoted outside normal business hours.

To achieve this efficiently, we'll introduce the concept of a *time series index*.
...
So we have almost 92K missing prices, which represent a little over 4% of the total data. Therefore, it is acceptable if we impute the prices. We also have missing data for `volume` and number of trades (`num_trades`). These can be imputed easily: substituting a zero everytime there's no price data (this is a reasonable imputation). First, let's check how many missing values we have per column:
...
```{r}
# Impute volume and num_trades
ipc_intraday <- ipc_intraday %>% 
  mutate(
    volume = if_else(is.na(last_price), 0, volume),
    num_trades = if_else(is.na(last_price), 0, num_trades)
  ) 
