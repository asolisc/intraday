...
RV_means %>% 
  ...
  geom_line(colour = blog_colors[2], 
             size = 1.2) +
...
$$ We now forecast a variance for the next month using the HAR model:

$$
\sigma^2_{t+1} = \alpha + \beta_1RV_{1d} + \beta_2RV_{1w} +\beta_3RV_{1m} + \gamma VIX^2_t/252 + u_i
$$
...
tictoc::tic()
vol_tuned <- vol_wf %>% 
  tune_grid(vol_resamples, grid = 25)
tictoc::toc()
