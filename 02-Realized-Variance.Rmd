...
We now forecast a variance for the next month using the HAR model. We use the same rule of thumb employed for estimating GARCH models with daily data: using the past 1,000 observations (which contains a good amount of data but reduces the risk of forecasting through a structural break).
...
vol_ts <- rv_for_modeling %>% 
  mutate(
    ...
    # RV variables
    rv_daily   = RV,
    rv_weekly  = (cum_rv - lag(cum_rv, n = 5L)) / 5,
    rv_monthly = (cum_rv - lag(cum_rv, n = 21L)) / 21,
    rv_target  = (lead(cum_rv, n = 21L) - cum_rv) / 21
  ) %>% 
  left_join(
    vol_index %>% 
      select(date, close) %>% 
  ...
  # Reorder columns
  select(index_date, rv_target, rv_daily, rv_weekly, rv_monthly, vix, year, month, day_id) %>% 
  
  drop_na(rv_monthly, rv_target)

```

How to impute the VIX data?
